...
public class UpdateCSVonDB {
    ...
    public UpdateCSVonDB() {
        ...
        database = "";
        csv = "";
        funcao = new Keys.Function("Pofessor");
        belementos = false;
    }
    ...
    public UpdateCSVonDB(TimeDate.Date inicio, TimeDate.Date fim, TimeDate.BreakPeriodList ferias, TimeDate.HolidaysList feriados, String database, String csv) {
        ...
        this.csv = csv;
        this.funcao = new Keys.Function("Pofessor"); 
    ...
    public void update() {
        if ((!database.equals("")) && (!csv.equals(""))) {
            ...
            if (db.isTie()) {
                ...
                Set<Keys.Request> requests = new TreeSet<>();
                if (elementos.size() > 0) {
                    belementos = true;
                    int i = 0;
                    boolean bauxiliar = false;
                    Set<Keys.Person> pessoas = new java.util.TreeSet<>();
                    Set<Keys.Material> materiais = new java.util.TreeSet<>();
                    Set<Keys.Subject> disciplinas = new java.util.TreeSet<>();
                    Set<Keys.ClassStudents> turmas = new java.util.TreeSet<>();
                    ...
                    while (i < elementos.size()) {
                        ...
                        if (datass.size() > 0) {
                            for (int j = 0; j < datass.size(); j++) {
                                ...
                                if (!bauxiliar) {
                                    Keys.Person pes = aux.getPerson();
                                    Keys.Material sala = aux.getClassRoom();
                                    Keys.Subject disciplina = aux.getSubject();
                                    Keys.ClassStudents turma = aux.getStudentsClass();
     ...
     * @return the funcao
     */
    public Keys.Function getFunction() {
        if (funcao == null) funcao = new Keys.Function();
        return funcao;
    }
     ...
     */
    public void setFunction(Keys.Function funcao) {
        this.funcao = funcao;
    }
