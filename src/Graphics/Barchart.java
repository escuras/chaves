...
public class Barchart extends javax.swing.JFrame {
    ...
    public Barchart(String title,String xtitle, String ytitle, java.util.List<Statistic.UserNumberMaterial> valores, Langs.Locale lingua, int quantidade) {
        super(title);
    ...
    public void createGraphic(String titulo, int largura, int altura) {
        ...
        grafico = ChartFactory.createBarChart(titulo, xtitle, ytitle, data, PlotOrientation.HORIZONTAL, true, false, false);
        this.altura = altura;
    ...
    private CategoryDataset createDataset() {
        ...
        for (int h = 0; h < valores.size(); h++) {
            dataset.setValue(valores.get(h).getNumber(),valores.get(h).getPerson().getName(),lingua.translate(valores.get(h).getType().getTypeOfMaterialName()));
            if (h == quantidade - 1) {
                break;
            }
        }
